...
2.	Sul **server** dell’applicazione, in questo caso viene memorizzata **su un file** del modulo che genera le pagine lato server (PHP, Python, ecc.). Sono le **sessioni** propriamente dette. **Il clientID**, analogo al bigliettino di un guaradaroba, è sempre memorizzato in un **cookie** sul browser del client. Una qualsiasi **struttura dati** come un oggetto o un'**autorizzazione di accesso**, con il **livello** associato a quell'iD, può invece essere memorizzata in una **lista sul server** in cui la **chiave di ricerca** è proprio il ClientID. Questo viene **ricevuto** dal server insieme al cookie contenuto nella **richiesta HTTP** della risorsa ed è utilizzato per recuperare i dati associati all'utente.
.
![session](session-php.gif)

4.	Sul **DBMS** all’interno delle tabelle che memorizzano i dati persistenti di una applicazione. Il **ClientID** o un **token** inviati insieme alla richiesta HTTP possono essere utilizzati come chiave di ricerca per recuperare dal DB le informazioni ad essi associate.
...
7.	Mediante **riscrittura dell'url**. E' un metodo analogo a quello dei campi nascosti soltanto che adesso il valore del sessionID viene inviato all'inizio dal server e poi palleggiato tra server e client all'interno dell'header dei pacchetti HTTP, in particolare nella zona riservata all'indirizzo url
...
Dal punto di vista della **sicurezza** memorizzare un'**autorizzazione di accesso** ad una risorsa sensibile (al limte la stessa autenticazione al sito) non è la stessa cosa che memorizzare **dati provvisori di navigazione**, anche se, in questo caso, la loro **privacy** andrebbe comunque attentamente valutata.
